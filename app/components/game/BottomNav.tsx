...
export default function BottomNav({ lang, labels }: Props) {
  ...
  const subBtnClass = (active: boolean, primary?: boolean) =>
    `flex items-center justify-center gap-2 rounded-full whitespace-nowrap shadow-lg transition-colors duration-150 ${
      primary ? 'px-5 py-3 text-base font-extrabold tracking-wide' : 'px-4 py-2.5 text-sm font-bold min-w-[7.5rem]'
    } ${
      active
  ...
  const sideBtnClass = (active: boolean) =>
    `flex items-center justify-center gap-2 px-5 py-3 rounded-full font-bold whitespace-nowrap shadow-lg transition-colors duration-150 min-w-[8.5rem] ${
      active
        ? 'bg-[#F5A623] text-[#4E342E]'
        : 'bg-[#4E342E] text-white hover:bg-[#5D4037]'
    }`

  // Arc positions for sub-buttons (relative to Hrát center)
  const subPositions = [
    { x: 0, y: -140 },     // Kampaň — peak (top center)
    { x: -120, y: -90 },   // Svět — down-left
    { x: 120, y: -90 },    // Freeplay — down-right
  ]
  ...
  return (
    <div ref={navRef} className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      {/* Flex container for three independent buttons */}
      <div className="flex items-end gap-4">

        {/* Reels — left side button */}
        <div
        ...
        {/* Center — Hrát button + arc sub-buttons */}
        <div className="relative">
          {/* Sub-buttons — arc fan-out above Hrát */}
          {subItems.map((item, i) => (
            <Link
              key={item.path}
              href={item.href}
          ...
          <button
            ...
            className="flex items-center gap-2.5 px-10 py-5 rounded-full bg-[#FEDC56] text-[#4E342E] font-extrabold text-xl whitespace-nowrap shadow-xl transition-all duration-300 ease-out hover:bg-[#f5d430]"
          >
